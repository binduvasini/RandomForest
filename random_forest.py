...
class random_forest(classifier):
    ...
    def __init__(self,trees=10,maxdepth=-1):
        self.tree = dict()
        self.trees = trees
    ...
    def choose_feature(self, X, Y, feature_list):
        crit = self.gini(Y)
    ...
    def build_tree(self, X, Y, feature_list):
        ...
        best_feature = self.choose_feature(X, Y,feature_list)
        this_tree = dict()
        ...
        for value in unique_values:
            ...
            this_tree[best_feature][value] = self.build_tree(subtree_x, subtree_y, feature_list)
        return this_tree
    ...
    def fit(self, X, Y, feature_list):
        self.tree = self.build_tree(X, Y, feature_list)
        return self.tree
